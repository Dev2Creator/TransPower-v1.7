...
async function loadTransformers() {
  ...
  try {
    const transformers = await import(workerBaseUrl + 'transformers-v3.min.js');
...
async function initModel() {
  ...
  if (!pipeline) {
    postBrainStatus(
      'error',
      `Sweetheart could not initialize Transformers.js locally: ${transformersLoadError?.message || 'pipeline export missing'}`
    );
    return;
  }
